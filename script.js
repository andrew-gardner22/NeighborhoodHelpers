function checkStoreHash() {
  ...
  if (window.location.hash === '#store') { overlay.classList.add('active'); document.body.style.pointerEvents = 'none'; overlay.style.pointerEvents = 'auto'; }
  else { overlay.classList.remove('active'); document.body.style.pointerEvents = 'auto'; }
}

function handleStoreBackBtn() {
  if (document.referrer || window.history.length > 1) { window.history.back(); }
  else { window.location.hash = '#home'; }
  setTimeout(() => { if (window.location.hash === '#store') window.location.hash = '#home'; }, 100);
}

window.addEventListener('hashchange', checkStoreHash);
window.addEventListener('DOMContentLoaded', checkStoreHash);
checkStoreHash();
window.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    logo.addEventListener('animationend', () => { logo.classList.add('spin-custom'); }, { once: true });
    setTimeout(() => { loader.style.opacity = 0; setTimeout(() => loader.remove(), 1000); }, 2500);
  }, 2500);
});
...
function showPage(hash) {
  ...
  if (hash.startsWith('#service/')) {
    ...
    if (section) { section.style.display = 'block'; showCouponIfApplicable(); }
  }
}
...
function initServices() {
  ...
  services.forEach(service => {
    ...
    `;
    if (service.title.toLowerCase() === "lawncare") { card.onclick = () => window.location.href = "https://andrew-gardner22.github.io/rrlawncare/"; }
...
function bookService(serviceName) {
  ...
  setTimeout(() => { const messageBox = document.querySelector('.contact-form textarea'); if (messageBox) messageBox.value = `I'm interested in booking: ${serviceName}`; }, 100);
}
...
function initForm() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    setTimeout(() => { status.textContent = "Message sent! We'll be in touch soon."; form.reset(); }, 1500);
  });
}

function buyProduct(name, price) {
  location.hash = '#contact';
  setTimeout(() => { const messageBox = document.querySelector('.contact-form textarea'); if (messageBox) messageBox.value = `I would like to buy ${name} for ${price}.`; }, 200); // delay ensures page has loaded
}
